...
public class PropertyResource {
    ...
    private final String apiUrl = "http://api.zoopla.co.uk/api/v1/property_listings.js" +
            "?api_key=gzj2nbsj5cy9x3b5gqwdtake&";
    private final String areaQuery = "area=";
    private final String listingQuery = "listing_id=";
    ...
    @Path("area/{area}")
    @Produces(MediaType.TEXT_PLAIN)
    public String getPropertiesIn(@PathParam("area") String area) {
        String query = apiUrl + areaQuery + area;
    ...
    @Path("{listingId}")
    @Produces(MediaType.TEXT_PLAIN)
    public String getPropertyDetails(@PathParam("listingId") Long listingId) {
        String query = apiUrl + listingQuery + listingId;
